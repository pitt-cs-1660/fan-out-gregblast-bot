...
def lambda_handler(event, context):
    ...
    for record in event['Records']: # todo: loop through event['Records']
        ...
        for s3_record in s3_event['Records']: # todo: loop through the S3 event's 'Records'
            ...
            size = s3_record['s3']['object']['size'] # todo: extract file size from s3_record['s3']['object']['size']
            event_time = s3_record['eventTime'] # todo: extract event time from s3_record['eventTime']
            ...
            s3.put_object(Bucket=bucket, Key=f"processed/metadata/{filename}.json", Body=json.dumps(metadata), ContentType='application/json')
